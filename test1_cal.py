...
fps = 150.0
step = 3
FRE = 85  # todo eachF [50,85,5]没有80，手动排着输
...
# path2 = 'D:\\guan2019\\1_disk\\a_left\\'+str(ACC)+'\\data\\'  # TODO: eachA
...
path3 = 'D:\\guan2019\\1_disk\\f\\'+str(FRE)+'Hz\\analysis_delta2\\'  # TODO: eachF !!!!! 改了step一定要改输出文件夹
# path3 = 'D:\\guan2019\\1_disk\\a_left\\'+str(ACC)+'\\analysis_delta2\\'  # TODO: eachA
...
for i in range(len(file_n)):
    ...
    plt.title('Trajectory '+' [' + str(FRE) +'$Hz$, '+ str(frame_name) +'$g$, ' + str(round(max_time)) +'$s$]', fontsize=10)  # todo eachF
    # plt.title('Trajectory '+' [' + str(frame_name) +'$Hz$, '+ str(ACC) +'$g$, ' + str(round(max_time)) +'$s$]', fontsize=10)  # todo eachA
    ...
    ax0 = plt.subplot(311)
    plt.plot(time, x, color='yellowgreen')
    ax0.set_xlabel('time(s)')
    ax0.set_ylabel('X(pixel)')
    ax0.grid(which='major', axis='x', linewidth=0.75, linestyle='-', color='0.75')  # 由每个x主坐标出发对x主坐标画垂直于x轴的线段
    plt.title('Fluctuation ' + ' [' + str(FRE) + '$Hz$, ' + str(frame_name) + '$g$, ' + str(round(max_time)) + '$s$]',fontsize=10)  # todo eachF
    ...
    ax2.grid(which='major', axis='x', linewidth=0.75, linestyle='-', color='0.75')  # 由每个x主坐标出发对x主坐标画垂直于x轴的线段
