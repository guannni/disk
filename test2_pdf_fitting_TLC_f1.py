...
for p in range(len(file_n)):#2,3):# # TODO:60Hz——range(2,len(file_n))
    ...
    for i in range(len(file_n1)):#2,3):#
        ...
        jump = np.array([1,15,30,100,150]) #[1,3,5,10,15,30,50,100,150] [1,30,50,100,150][1,2,5,10,15]
    ...
    for i in range(len(filename_1)):#2,3):#
        ...
        for step in jump:
            ...
            if step < 150 and step!=30:
                ...
                ax1.scatter(0.8*pdf_trans_dict[filename_1[i] + 'x'+str(step)], 1000*pdf_trans_dict[filename_1[i] + 'y'+str(step)],c='',  marker=marker[j], s = 25, edgecolor=colors[j], label=str(frac_reduc(150.0,step)[1])+'/'+str(frac_reduc(150.0,step)[0])+' s')
                ...
                middle -= 1
                pdf_trans_dict[filename_1[i] + 'x' + str(step)] -= pdf_trans_dict[filename_1[i] + 'x' + str(step)][middle]
                # ax1.scatter(pdf_trans_dict[filename_1[i] + 'x'], np.clip(pdf_trans_dict[filename_1[i] + 'y'], 1e-18, None)/2, alpha=0.75,color=colors[i], label=label[i])
                ax1.scatter(0.8*pdf_trans_dict[filename_1[i] + 'x' + str(step)],1000*pdf_trans_dict[filename_1[i] + 'y'+str(step)], c='', alpha=0.65,marker=marker[j], s = 25,edgecolor=colors[j],label=str(frac_reduc(150.0,step)[1])+'/'+str(frac_reduc(150.0,step)[0])+' s')
                # marker[j], s = 25     'o',s=10
            else:
                # ax1.scatter(pdf_trans_dict[filename_1[i] + 'x'], np.clip(pdf_trans_dict[filename_1[i] + 'y'], 1e-18, None)/2, alpha=0.75,color=colors[i], label=label[i])
                ax1.scatter(0.8*pdf_trans_dict[filename_1[i] + 'x' + str(step)],1000*pdf_trans_dict[filename_1[i] + 'y'+str(step)], c='', alpha=0.65,marker=marker[j], s = 25,edgecolor=colors[j],label=str(np.around(step/150.0, decimals=1)) + ' s')
                # marker[j], s = 25     'o',s=10
            ...
            y1 = pdf_trans_dict[filename_1[i] + 'y'+str(step)][middle:1:-1]   # ExponentialModel()
            ...
            y2 = pdf_trans_dict[filename_1[i] + 'y'+str(step)][middle:-1]  # ExponentialModel()
            ...
            ax1.plot(-xnew[::-1]*0.8, 1000*np.array(y11[::-1]), color=colors[j],alpha=0.6)
            ax1.plot(0.8*np.array(xnew), 1000*np.array(y11), color=colors[j], alpha=0.6)
            # 60Hz *5000; 100Hz *1000
        ...
        if filename_1[i] == '9100_2':
            ...
            expo1_y = [2.13885054, 1.90941303,  1.94897102, 1.92868116,1.88936307]
        ...
        elif filename_1[i] == '60_3':
            left, bottom, width, height = [0.68, 0.715, 0.19, 0.15]
            ax3 = fig.add_axes([left, bottom, width, height])
            expo1_y = [1.13479151, 1.25524046, 1.39937185, 1.72221259, 1.92892126]
            expo1_x = np.array([1,15,30,100,150])/150.0#[1, 2, 5, 10,15]  # [np.around(1/150.0, decimals=2), np.around(2/150.0, decimals=2), np.around(5/150.0, decimals=2),np.around(10/150.0, decimals=2), np.around(15/150.0, decimals=2)]
            ax3.scatter(expo1_x, expo1_y, s=12)
            ...
            # ax3.set_xlim(0, 17)
            ax3.set_ylim(1,2)
            ax3.set_xlabel(r'$\Delta t$ (s)', fontsize=9)
            ax3.set_ylabel(r'$\beta$', fontsize=9)
            ax3.xaxis.set_label_coords(0.6, -0.32)
            ax3.yaxis.set_label_coords(-0.32, 0.5)
            ax3.tick_params(axis="x", direction="in", labelsize=8.8)
            ax3.tick_params(which='minor', direction='in')
            ax3.tick_params(axis="y", direction="in", labelsize=9)
    ...
    for i in range(len(filename_1)):#2,3):#
        ...
        for step in jump:
            ...
            print(middle)
            if step>=150:
                ax2.scatter(pdf_rot_dict[filename_1[i] + 'x'+str(step)], 0.01*smooth(pdf_rot_dict[filename_1[i] + 'y'+str(step)])[1:len(pdf_rot_dict[filename_1[i] + 'x' + str(step)])+1], c='', alpha=0.65,marker=marker[j], s = 25 , edgecolor=colors[j], cmap='hsv', label= str(np.around(step/150.0, decimals=1))+' s')
                # marker[j], s = 25     'o',s=10
                ...
                ax2.scatter(pdf_rot_dict[filename_1[i] + 'x' + str(step)],0.01*smooth(pdf_rot_dict[filename_1[i] + 'y' + str(step)])[1:len(pdf_rot_dict[filename_1[i] + 'x' + str(step)])+1], c='',marker=marker[j], s = 25, edgecolor=colors[j],cmap='hsv', label=str(frac_reduc(150.0,step)[1])+'/'+str(frac_reduc(150.0,step)[0])+' s')
                # marker[j], s = 25     'o',s=10
                # pdf_rot_dict[filename_1[i] + 'x'+str(step)]-pdf_rot_dict[filename_1[i] + 'x'+str(step)][middle]
        ...
        # # GaussianModel()------------
                ...
                ax2.plot(xnew, 0.01*ynew,color=colors[j],alpha=0.5)
                # 60Hz 0.01*;100Hz 无
