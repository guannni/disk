# 读取hdf文件，计算pdf
# 出D:\guan2019\1_disk\active\下 按mode分类的数据的pdf图像
...
def add_text(x,y,data):
    for x0,y0,data0 in zip(x,y,data):
        ax[0].text(x0,y0,data0)#,rotation=90)
...
delta_dict = {}
label = []
for p in range(len(filename)):#2,3):# # TODO:60Hz——range(2,len(file_n))
    path_3 = path2 + filename[p] + '\\'
    ...
    for i in range(len(file_n1)):#2,3):#
        ...
        au, bu, cu = plt.hist(deltar,[x*0.06+0.03 for x in range(-39,40)], histtype='bar', facecolor='yellowgreen', alpha=0.75, rwidth=1)#, density=True) ###active&inactive_select  # au是counts，bu是deltar
        # au, bu, cu = plt.hist(deltar, histtype='bar', facecolor='yellowgreen', alpha=0.75, rwidth=1) ###inactive
        au_ind = np.where(au == 0)
        au = np.delete(au,au_ind)
        # au1 = np.array([math.log(x) for x in au])
        bu = (bu[:-1]+bu[1:])/2.
        bu = np.delete(bu,au_ind)
        pdf_trans_dict[filename[p]+'x'] = bu  # 存入dict

        pdf_trans_dict[filename[p] + 'y'] = au/len(deltar) #- np.min(au1)



        # /np.sqrt(np.sum(deltatheta ** 2)/len(deltar))/0.001/1.4/10**(-23)
        # [x*0.026+0.013 for x in range(-29,30)]
        AU, BU, CU = plt.hist(deltatheta,[x*0.026+0.013 for x in range(-49,50)],   histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True) ###active&inactive_select  /(np.sqrt(2*np.sum(deltatheta**2)/len(deltatheta)))
        # AU, BU, CU = plt.hist(deltatheta,   histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)###inactive
        AU_ind = np.where(AU == 0)
        AU = np.delete(AU, AU_ind)
        pdf_rot_dict[filename[p] + 'y'] = AU/len(deltar)
        BU = (BU[:-1]+BU[1:])/2.
        BU = np.delete(BU, AU_ind)
        pdf_rot_dict[filename[p]+ 'x'] = BU  # 存入dict
        ...
        #energy---------
        # range=(0, 0.00006),
        aue, bue, cue = plt.hist(0.001*(dr/1000*150)**2,65, range=(0, 0.00006),  histtype='bar', facecolor='yellowgreen', alpha=0.75, rwidth=1)#, density=True)  # au是counts，bu是deltar
        aue_ind = np.where(aue == 0)
        aue = np.delete(aue, aue_ind)
        pdf_transenergy_dict[filename[p]+'y'] = aue/len(dr)
        bue = (bue[:-1]+bue[1:])/2.
        bue = np.delete(bue, aue_ind)
        pdf_transenergy_dict[filename[p]+'x'] = bue  # 存入dict

        

        #(0, 0.003)
        AUE, BUE, CUE = plt.hist(0.0000002*(deltatheta*150)**2/16, 65, range=(0, 0.00006), histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True)
        AUE_ind = np.where(AUE == 0)
        AUE = np.delete(AUE, AUE_ind)
        pdf_rotenergy_dict[filename[p] + 'y'] = AUE/len(dr)
        BUE = (BUE[:-1]+BUE[1:])/2.
        BUE = np.delete(BUE, AUE_ind)
        pdf_rotenergy_dict[filename[p] + 'x'] = BUE  # 存入dict

        delta_dict[filename[p]+'r'] = deltar
        delta_dict[filename[p]+'theta'] = deltatheta


        # total energy ---
        AUTE, BUTE, CUTE = plt.hist(0.001*(dr/1000*150)**2+0.0000002*(deltatheta*150)**2/16,65,range=(0,0.00006), histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True)
        AUTE /= len(dr)
        AUTE_ind = np.where(AUTE == 0)
        AUTE = np.delete(AUTE, AUTE_ind)
        BUTE = (BUTE[:-1] + BUTE[1:]) / 2.
        BUTE = np.delete(BUTE, AUTE_ind)
        pdf_energy_dict[filename[p]+'x'] = BUTE  # 存入dict
        pdf_energy_dict[filename[p] + 'y'] = AUTE  # - np.min(au1)
        print(filename[p])
    ...
    ys = [i + x + (i * x) ** 2 for i in range(len(filename))]
    colors = cm.rainbow(np.linspace(0, 1, len(ys)))
    marker = ['o', 'v', 'D', '^', 's', 'h', 'p', '*', '+', 'x',1,3]
...
for i in range(len(filename)):#2,3):#
    ax1.scatter(pdf_trans_dict[filename[i] + 'x'], pdf_trans_dict[filename[i] + 'y'], marker=marker[i],  c='', s=25, edgecolor=colors[i], cmap='hsv', label=label[i])#, label=label[i])
    # ax1.errorbar(pdf_trans_dict[filename[i] + 'x'], pdf_trans_dict[filename[i] + 'y'],yerr=err_trans_dict[filename[i]],fmt='none',elinewidth=1,ms=1,ecolor=colors[i])
    # middle = np.where(pdf_trans_dict[filename[i] + 'x'] == find_nearest(pdf_trans_dict[filename[i] + 'x'], 0))[0][0]  # pdf 0 的点    # x = np.delete(pdf_rot_dict[filename[i] + 'x'],middle-1)
    # # if float(filename[i].split('_', 1)[0]) == 4.0:
    # #     x = pdf_trans_dict[filename[i] + 'x'][middle+1:]
    # #     y = pdf_trans_dict[filename[i] + 'y'][middle+1:]
    # # else:
    # #     x = pdf_trans_dict[filename[i] + 'x'][middle:]
    # #     y = pdf_trans_dict[filename[i] + 'y'][middle:]
    # x = pdf_trans_dict[filename[i] + 'x'][middle-1:]
    # sig = err_trans_dict[filename[i]][middle-1:]
    # y = pdf_trans_dict[filename[i] + 'y'][middle-1:]
    # popt, pcov = curve_fit(func, x, y, maxfev=10000,sigma=1./np.array(sig), absolute_sigma=True)
    # x1 = pdf_trans_dict[filename[i] + 'x'][middle+1::-1]
    # sig = err_trans_dict[filename[i]][middle+1::-1]
    # y1 = pdf_trans_dict[filename[i] + 'y'][middle+1::-1]
...
for i in range(len(filename)):#2,3):#
    # sns.distplot(delta_dict[filename[i] + 'theta'], norm_hist=True,bins=100, kde=True, hist=False,label=label[i])  #'mode'+str(i+1))#
    ax2.scatter(pdf_rot_dict[filename[i] + 'x'], pdf_rot_dict[filename[i] + 'y'],marker=marker[i],  c='', s=25, edgecolor=colors[i], cmap='hsv', label=label[i])
    # ax2.errorbar(pdf_rot_dict[filename[i] + 'x'], pdf_rot_dict[filename[i] + 'y'],yerr=err_rot_dict[filename[i]], fmt='none', elinewidth=1, ms=1, ecolor=colors[i])
    # x = pdf_rot_dict[filename[i] + 'x'] * 100.0
    # y = pdf_rot_dict[filename[i] + 'y']

    # if float(filename[i].split('_', 1)[0]) == 3.5:
    #     popt, pcov = curve_fit(funcer0, x, y, maxfev=100000,sigma=1./np.array(err_rot_dict[filename[i]]), absolute_sigma=True)  # ,bounds=([0,0,1], [np.inf, np.inf, 1.01]))
    #     print(popt)
    ...
    #     popt, pcov = curve_fit(funcer1, x, y, maxfev=100000,sigma=1./np.array(err_rot_dict[filename[i]]), absolute_sigma=True)  # ,bounds=([0,0,1], [np.inf, np.inf, 1.01]))
    #     print(popt)
    ...
    #     popt, pcov = curve_fit(funcer2, x, y, maxfev=100000,sigma=1./np.array(err_rot_dict[filename[i]]), absolute_sigma=True)  # ,bounds=([0,0,1], [np.inf, np.inf, 1.01]))
    #     print(popt)
    #     xnew = np.linspace(x.min()-3, x.max()+3, 10000)  # 平滑画曲线
    #     y11 = [funcer2(p, popt[0], popt[1], popt[2], popt[3], popt[4]) for p in xnew]
    #     print(filename[i])
    # elif float(filename[i].split('_', 1)[0]) == 5.0:
    #     popt, pcov = curve_fit(funcer3, x, y, maxfev=100000,sigma=1./np.array(err_rot_dict[filename[i]]), absolute_sigma=True) # ,bounds=([0,0,1], [np.inf, np.inf, 1.01]))
    #     print(popt)
...
transenergy_ave =[]
for i in range(len(filename)):#2,3):#
    transenergy_ave.append(sum(1000000*pdf_transenergy_dict[filename[i] + 'x']*np.clip(pdf_transenergy_dict[filename[i] + 'y'], 1e-18, None)))# uJ
    print(filename[i],'t',transenergy_ave[i])
    ax1.scatter(pdf_transenergy_dict[filename[i] + 'x']*1000, np.clip(pdf_transenergy_dict[filename[i] + 'y'], 1e-18, None), marker=marker[i], s=25, c='', edgecolors=colors[i], label=label[i]) #mJ
    # ax1.errorbar(pdf_transenergy_dict[filename[i] + 'x'] * 1000, pdf_transenergy_dict[filename[i] + 'y'],yerr=err_trans_energy_dict[filename[i]], fmt='none', elinewidth=1, ms=1, ecolor=colors[i])
    # if float(filename[i].split('_', 1)[0]) == 3.5:
    #     x = pdf_transenergy_dict[filename[i] + 'x'][1:] * 10000
    #     y = pdf_transenergy_dict[filename[i] + 'y'][1:] * 10
    # elif float(filename[i].split('_', 1)[0]) == 4.0:
    #     x = pdf_transenergy_dict[filename[i] + 'x'][1:] * 10000
    #     y = pdf_transenergy_dict[filename[i] + 'y'][1:] * 10
    # elif float(filename[i].split('_', 1)[0]) == 4.5:
    ...
    # xnew = np.linspace(0.0001, np.array(pdf_transenergy_dict[filename[i] + 'x'] * 10000).max(), 100)  # 平滑画曲线
...
rotenergy_ave = []
for i in range(len(filename)):  # 2,3):#
    rotenergy_ave.append(sum(1000000*pdf_rotenergy_dict[filename[i] + 'x']*np.clip(pdf_rotenergy_dict[filename[i] + 'y'], 1e-18, None))) # uJ
    print(filename[i],'r',rotenergy_ave[i])
    ax2.scatter(pdf_rotenergy_dict[filename[i] + 'x']*1000, np.clip(pdf_rotenergy_dict[filename[i] + 'y'], 1e-18, None),  marker=marker[i], s=25, c='', edgecolors=colors[i], label=label[i])   # mJ
    # ax2.errorbar(pdf_rotenergy_dict[filename[i] + 'x'] * 1000, pdf_rotenergy_dict[filename[i] + 'y'],yerr=err_rot_energy_dict[filename[i]], fmt='none', elinewidth=1, ms=1, ecolor=colors[i])
    # if float(filename[i].split('_', 1)[0]) == 3.5:
    #     x = pdf_rotenergy_dict[filename[i] + 'x'][1:] * 10000
    #     y = pdf_rotenergy_dict[filename[i] + 'y'][1:] *10
    # elif float(filename[i].split('_', 1)[0]) == 4.0:
    #     x = pdf_rotenergy_dict[filename[i] + 'x'][1:] * 10000
    #     y = pdf_rotenergy_dict[filename[i] + 'y'][1:]*10
    # elif float(filename[i].split('_', 1)[0]) == 4.5:
    ...
    # xnew = np.linspace(x.min(), np.array(pdf_rotenergy_dict[filename[i] + 'x'] * 10000).max(), 100)  # 平滑画曲线
    # y11 = [funce(p, popt[0], popt[1], popt[2]) for p in xnew]
...
totalenergy_ave = []
for i in range(len(filename)):#2,3):#
    totalenergy_ave.append(sum(1000000*pdf_energy_dict[filename[i] + 'x']*np.clip(pdf_energy_dict[filename[i] + 'y'], 1e-18, None))) # uJ
    print(filename[i],'total',totalenergy_ave[i])
    ax3.scatter(pdf_energy_dict[filename[i] + 'x']*1000,np.clip(pdf_energy_dict[filename[i] + 'y'], 1e-18, None),  marker=marker[i], s=25, c='', edgecolors=colors[i]) # mJ
    
    # ax3.errorbar(pdf_energy_dict[filename[i] + 'x'] * 1000, pdf_energy_dict[filename[i] + 'y'],yerr=err_energy_dict[filename[i]], fmt='none', elinewidth=1, ms=1, ecolor=colors[i])
    # if float(filename[i].split('_', 1)[0]) == 3.5:
    #     x = pdf_energy_dict[filename[i] + 'x'][2:-3] * 10000
    #     y = pdf_energy_dict[filename[i] + 'y'][2:-3] *10
    # elif float(filename[i].split('_', 1)[0]) == 4.0:
    #     x = pdf_energy_dict[filename[i] + 'x'][2:-3] * 10000
    #     y = pdf_energy_dict[filename[i] + 'y'][2:-3]*10
    # elif float(filename[i].split('_', 1)[0]) == 4.5:
    ...
    #     y = pdf_energy_dict[filename[i] + 'y'][2:-3]*10
    # elif float(filename[i].split('_', 1)[0]) == 5.0:
    #     x = pdf_energy_dict[filename[i] + 'x'][2:-7]* 10000
    #     y = pdf_energy_dict[filename[i] + 'y'][2:-7]*10
    # popt, pcov = curve_fit(funce, x, y, maxfev=100000)
    # xnew = np.linspace(x.min(), np.array(pdf_energy_dict[filename[i] + 'x'] * 10000).max(), 100)  # 平滑画曲线
    # y11 = [funce(p, popt[0], popt[1], popt[2]) for p in xnew]
...
# energy average bar plot-----
index = np.arange(len(filename))
width = 0.5

fig,ax = plt.subplots(1,2,figsize=(10,4))
ax[0].bar(filename, transenergy_ave, width=0.5, label='$E_t$')  # uJ
ax[0].bar(filename, rotenergy_ave, width=0.5, bottom= transenergy_ave,label='$E_r$')  # uJ
leg = ax[0].legend(loc='upper right')
leg.get_frame().set_linewidth(0.0)
# ax[0].xaxis.set_minor_locator(MultipleLocator(0.005))
ax[0].tick_params(axis="x", direction="in")
ax[0].tick_params(which='minor', direction='in')
ax[0].tick_params(axis="y", direction="in")
ax[0].set_ylabel('$E$ (\u03BC J)')
ax[0].set_xlabel('$f$ (Hz)')
# ax[0].set_ylim((0.00005, 4))
# ax[0].set_xlim((-0.0002,0.034))
# ax[0].set_yscale('log')


transenergy_percentage = np.array(transenergy_ave)/(np.array(transenergy_ave)+np.array(rotenergy_ave) ) # totalenergy_ave # percentage of energy
rotenergy_percentage = np.array(rotenergy_ave)/(np.array(transenergy_ave)+np.array(rotenergy_ave) )  # totalenergy_ave

t_per_string = ['%.1f%%' % round(i*100,3) for i in transenergy_percentage]
r_per_string = ['%.1f%%' % round(i*100,3) for i in rotenergy_percentage]
add_text(index-width/2,np.array(transenergy_ave)/2, t_per_string)
add_text(index-width/2,np.array(transenergy_ave)+np.array(rotenergy_ave)/2.0, r_per_string)


ax[1].bar(filename, transenergy_percentage, width=0.4, label='$E_t$')
ax[1].bar(filename,rotenergy_percentage, width=0.4, bottom= transenergy_percentage,label='$E_r$')
...
# ax[1].set_ylim((0.00005, 4))
# ax[1].set_xlim((-0.0002,0.034))
# ax[1].set_yscale('log')
