...
for p in range(len(file_n)):#2,3):# # TODO:60Hz——range(2,len(file_n))
    ...
    for i in range(len(file_n1)):#2,3):#
        ...
        AU, BU, CU = plt.hist(deltatheta,71,   histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True)  /(np.sqrt(2*np.sum(deltatheta**2)/len(deltatheta)))
...
#energy---------
        # range=(0, 0.00006),
        aue, bue, cue = plt.hist(0.001*(dr/1000*150)**2,71,  histtype='bar', facecolor='yellowgreen', alpha=0.75, rwidth=1)#, density=True)  # au是counts，bu是deltar
        pdf_transenergy_dict[filename_1[i]+'y'] = aue/len(dr)
        bue = (bue[:-1]+bue[1:])/2.
        pdf_transenergy_dict[filename_1[i]+'x'] = bue  # 存入dict

        #(0, 0.003)
        AUE, BUE, CUE = plt.hist(0.0000002*(deltatheta*150)**2/16, 71,  histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True)
    ...
    for i in range(len(filename_1)):#2,3):#
        # sns.distplot(delta_dict[filename_1[i] + 'r'],norm_hist=True, bins=200, kde=True, hist=False, label=filename_1[i].split('_', 1)[0] + 'g')  #'mode'+str(i+1))#
        plt.scatter(pdf_trans_dict[filename_1[i] + 'x'], pdf_trans_dict[filename_1[i] + 'y'], alpha=0.75, color=colors[i], cmap='hsv', label=filename_1[i].split('_', 1)[0] + 'g')
        # plt.plot(pdf_trans_dict[filename_1[i] + 'x'], pdf_trans_dict[filename_1[i] + 'y'], linewidth=2, alpha=0.5,label=filename_1[i].split('_', 1)[0] + 'g')
        label.append(filename_1[i].split('_', 1)[0] + 'g')
...
# # # ExponentialModel()-------
#         if float(filename[i].split('_', 1)[0])==4.5:
#             x = pdf_trans_dict[filename_1[i] + 'x'][middle+4:]
#             y = pdf_trans_dict[filename_1[i] + 'y'][middle+4:] # ExponentialModel()
#         elif float(filename[i].split('_', 1)[0])==3.5:
#             x = pdf_trans_dict[filename_1[i] + 'x'][middle+2:]  # ExponentialModel()
#             y = pdf_trans_dict[filename_1[i] + 'y'][middle+2:]  # ExponentialModel()
#         else:
...
#         # ax1.legend(loc='best')
 ...
 #        mod = GaussianModel()  #ExponentialModel() # PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #VoigtModel()  #LorentzianModel()  #
 #        pars = mod.guess(y, x=x)
 #        out = mod.fit(y, pars, x=x)
 #        # print(out.fit_report(min_correl=0.25))
 #        ax1.plot(x, out.best_fit, color=colors[i], alpha=0.5)
...
# function fitting
    ...
    ax1.set_title('PDF' + ' [60Hz] ', fontsize=10)
    ...
    for i in range(len(filename_1)):#2,3):#
        ...
        plt.scatter(pdf_rot_dict[filename_1[i] + 'x'], pdf_rot_dict[filename_1[i] + 'y'],alpha=0.75, color=colors[i], cmap='hsv', label=filename_1[i].split('_', 1)[0] + 'g')
...
# 1----------
        ...
        pars1['l1_sigma'].set(value=1)
        pars1['l1_amplitude'].set(value=0.1)
        # pars1['l1_gamma'].set(value=5, min=1)  # ExponentialGaussianModel
        #

        lorentz2 = LorentzianModel (prefix='l2_')
        pars1.update(lorentz2.make_params())
        pars1['l2_center'].set(value=0)
        pars1['l2_sigma'].set(value=0.01)
        pars1['l2_amplitude'].set(value=0.001)
        # pars1['l2_gamma'].set(value=5, min=1)  # ExponentialGaussianModel
    ...
    # 2-------------
        # mod = VoigtModel()  # LorentzianModel()  # GaussianModel()  #PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #
        # pars = mod.guess(y, x=x)
        # out = mod.fit(y, pars, x=x)
        # print(out.fit_report(min_correl=0.25))
        #
