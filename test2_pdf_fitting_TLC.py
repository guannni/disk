...
# v_distribution用这个，能量还是用test2_pdf_fitting_TLC_log.py
...
def funcer0(x, a, b,c, d,e): # rotational 整体fit
    return 0.9**2* np.exp(-(x+b)**2 / c**2) + 0.9**2 * np.exp(-(x-b )**2 / c**2) +0.000006** 2  * np.exp(-x**2 / e**2)  # 3 Gaussian 2 * np.exp(-(x+0.1 )**2 / c**2) +2 * np.exp(-(x-0.1 )**2 / c**2) +
def funcer1(x, a, b,c, d,e): # rotational 整体fit
    return a**2 * np.exp(-(x - 7.9)**2 /c**2) + a**2* np.exp(-(x + 7.9)**2 / c**2) + d ** 2 *np.exp(-x**2 / e**2)  # 3 Gaussian
def funcer2(x, a, b, c, d, e):  # rotational 整体fit
    return 0.27**2 * np.exp(-(x - 12) ** 2 / 7.7** 2) +0.27**2 * np.exp(-(x + 12) ** 2 /7.7** 2) + 0.13  * np.exp(-x ** 2 / 3** 2)  # 3 Gaussian
def funcer3(x, a, b, c, d, e):  # rotational 整体fit
    return a**2 * np.exp(-(x - 18) ** 2 / 7.7** 2) + a**2 * np.exp(-(x + 18) ** 2 / 7.7** 2) +0.12** 2 * np.exp(-x ** 2 / e** 2)  # 3 Gaussian
...
err_trans_dict['9100_2']=[0.000265521,0.000245724,0.000127656,0.000278402,0.000279354,0.000540538,0.000518253,0.002128359,0.000754352,0.000670519,0.00101711,0.00114894,0.000496039,0.000177329,0.000242063,0.001177205,0.00092068,0.000840626,0.000558721,0.000299901,0.000298088,1.87033E-05,0.000458161,0.000652289,2.56759E-04,2.8703E-05]
...
# 有两种 bins=71  /31(log)，bins=31(点平滑,但是太少了,作图时需要/2来与71结果一致)，拟合用bins=71（点多，略凌乱，获得的结果的概率接近TL）
        ...
        #(0, 0.000003)
        AUE, BUE, CUE = plt.hist(0.0000002*(dtheta*150)**2/16, 45, range=(0, 0.00003), histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2)#, density=True)
    ...
    for i in range(len(filename_1)):#2,3):#
        ...
        ax1.scatter(pdf_trans_dict[filename_1[i] + 'x'], np.clip(pdf_trans_dict[filename_1[i] + 'y'], 1e-18, None)/2, marker=marker[i], c='', s=25, edgecolor=colors[i], label=label[i])
        ax1.errorbar(pdf_trans_dict[filename_1[i] + 'x'], pdf_trans_dict[filename_1[i] + 'y']/2.0,yerr=err_trans_dict[filename_1[i]],fmt='none',elinewidth=1,ms=1,ecolor=colors[i])
    ...
    # #     ax1.plot(x, out.best_fit/2,color=colors[i],alpha=0.5)
    # #     # ax1.legend(loc='best')
    # #
    ########## Translatioanl fitting 1 # function fitting
        ...
        xnew = np.linspace(x.min()-0.3, x.max()+0.3, 100) # 平滑画曲线
        y11 = [funce(p, popt[0], popt[1], popt[2]) for p in xnew]
        print(filename[i])
        print(popt) # 振幅要*1/2（第一个系数）
        print(pcov)
        ax1.plot(-xnew[::-1], np.array(y11[::-1])/2.0, color=colors[i],alpha=0.6)
        ax1.plot(xnew, np.array(y11)/2.0, color=colors[i],alpha=0.6)
    ...
    for i in range(len(filename_1)):#2,3):#
        ...
        ax2.scatter(pdf_rot_dict[filename_1[i] + 'x'], pdf_rot_dict[filename_1[i] + 'y']/2,  marker=marker[i],  c='', s=25, edgecolor=colors[i], cmap='hsv', label=label[i])
        ax2.errorbar(pdf_rot_dict[filename_1[i] + 'x'], pdf_rot_dict[filename_1[i] + 'y']/2.0,yerr=err_rot_dict[filename_1[i]],fmt='none',elinewidth=1,ms=1,ecolor=colors[i])
        ...
        if int(filename_1[i].split('_', 1)[0]) == 50:
            ...
            # a ** 2 * np.exp(-(x - 13) ** 2 / c ** 2) + a ** 2 * np.exp(-(x + 13) ** 2 / c ** 2) + d ** 2 * np.exp(-x ** 2 / e ** 2)
            x_80_1 = np.arange(-0.35,0.2,0.01)*100.0
            y_80_1 = [0.27 ** 2 * np.exp(-(x_i + 12) ** 2 / 7.7** 2) for x_i in x_80_1 ]
            ax2.scatter(x_80_1/100.0, np.array(y_80_1)/2.0,color='orange', alpha=0.6,s=3)
            x_80_2 = np.arange(-0.2,0.35,0.01)*100.0
            y_80_2 = [0.27 ** 2 * np.exp(-(x_i - 12) ** 2 / 7.7** 2) for x_i in x_80_2]
            ax2.scatter(x_80_2 / 100.0, np.array(y_80_2) / 2.0, color='orange', alpha=0.6, s=3)
            x_80_3 = np.arange(-0.15, 0.15, 0.0025) * 100.0
            y_80_3 = [0.13 * np.exp(-x_i ** 2 / 3** 2)  for x_i in x_80_3]
            ax2.scatter(x_80_3 / 100.0, np.array(y_80_3) / 2.0, color='orange', alpha=0.7, s=3)
        ...
        ax2.plot(xnew / 100.0, np.array(y11) /2.0, color=colors[i], alpha=0.6)
    ...
    #         y = np.delete(pdf_rot_dict[filename_1[i] + 'y'], middle-1)
    #         # x = np.delete(x, middle-2)
    #         # y = np.delete(y, middle-2)
    #         # x = np.delete(x, middle-3)
    #         # y = np.delete(y, middle-3)
    #         # x = np.delete(x, middle-4)
    #         # y = np.delete(y, middle-4)
    #         # x = np.delete(x, middle-5)
    #         # y = np.delete(y, middle-5)
    #
    ...
    #
    #         # 无
    #         pars1['l2_center'].set(value=-0.15, max=-0.05, min=-0.5)  # Exponential GaussianModel
    ...
    #         print(out1.fit_report(min_correl=0.5))
    #         # ax2.plot(x, y, 'bo-',alpha=0.6,marker='o',markersize=3)
    #         ax2.plot(x, out1.best_fit/2,color=colors[i],alpha=0.5)
    #         # plt.show()
    #         # ax2.legend(loc='best')
    #
    # ##################整组数据拟合 1（左右一起) <=60Hz ##############################
    ...
    #         pars1['l2_sigma'].set(value=0.1, min=0.01)
    ...
    ax5.scatter([50, 60, 80, 100], np.array(area_per), s=11)  # [50,60,80,100][6.0, 8.7, 15.5,24.0]
    ax5.set_xlim(40,110)
    ...
    ax5.set_ylabel(r'fraction',fontsize=9)
    ax5.xaxis.set_label_coords(0.35, -0.32)
    ax5.yaxis.set_label_coords(1.5,0.5)
...
#----energy distribution
    ...
    ax2 = fig.add_subplot(122)
    for i in range(len(filename_1)):#2,3):#
        ax1.scatter(pdf_transenergy_dict[filename_1[i] + 'x']*1000, np.clip(pdf_transenergy_dict[filename_1[i] + 'y'], 1e-18, None)/2, alpha=0.75,color=colors[i], label=label[i])
#
# ########## Translatioanl fitting 1
#
#         x = pdf_transenergy_dict[filename_1[i] + 'x']*1000 # ExponentialModel()
#         y = np.clip(pdf_transenergy_dict[filename_1[i] + 'y'], 1e-18, None)  # ExponentialModel()  ## # xlogy(np.sign(pdf_trans_dict[filename_1[i] + 'y']), pdf_trans_dict[filename_1[i] + 'y']) / np.log(math.e())
#
# # # # # ExponentialModel()---------
# #         mod = ExponentialModel()
# #         pars = mod.guess(y, x=x)
...
# #         print(popt)  # 振幅要*1/2（第一个系数）
# #         print(pcov)
# #         ax1.plot(x, np.array(y11) / 2, color=colors[i], alpha=0.5)
#
# # GaussianModel()------------
# #         mod = GaussianModel()  #PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #VoigtModel()  #LorentzianModel()  #ExponentialModel() #GaussianModel()  #PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #VoigtModel()  #LorentzianModel()  #
# #         pars = mod.guess(y, x=x)
...
# #         # ax1.legend(loc='best')
    ...
    for i in range(len(filename_1)):  # 2,3):#
        ax2.scatter(pdf_rotenergy_dict[filename_1[i] + 'x']*1000, np.clip(pdf_rotenergy_dict[filename_1[i] + 'y'], 1e-18, None) / 2, alpha=0.75, color=colors[i], label=label[i])

# ########## Rotational fitting 1
#         x = pdf_rotenergy_dict[filename_1[i] + 'x'] * 1000  # ExponentialModel()
#         y = np.clip(pdf_rotenergy_dict[filename_1[i] + 'y'], 1e-18,
#                     None)  # ExponentialModel()  ## # xlogy(np.sign(pdf_trans_dict[filename_1[i] + 'y']), pdf_trans_dict[filename_1[i] + 'y']) / np.log(math.e())
#
# # # # ExponentialModel()---------
# #         mod = ExponentialModel()
# #         pars = mod.guess(y, x=x)
...
#
# ########## Translatioanl fitting 1 # function fitting
#         # popt, pcov = curve_fit(func, x[0:30], y[0:30], maxfev=10000)
#         # y11 = [funce(p, popt[0], popt[1], popt[2]) for p in x]
#         # print(filename[i])
#         # print(popt)  # 振幅要*1/2（第一个系数）
#         # print(pcov)
#         # ax2.plot(x, np.array(y11) / 2, color=colors[i], alpha=0.5)
#
# # # GaussianModel()------------
# #         mod = GaussianModel()  #PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #VoigtModel()  #LorentzianModel()  #ExponentialModel() #GaussianModel()  #PseudoVoigtModel()  #SplitLorentzianModel()  # SkewedVoigtModel()  #VoigtModel()  #LorentzianModel()  #
# #         pars = mod.guess(y, x=x)
