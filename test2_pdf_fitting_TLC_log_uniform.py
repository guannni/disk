...
from lmfit.models import GaussianModel,Pearson7Model, LorentzianModel,LognormalModel, DampedOscillatorModel,ExponentialModel,VoigtModel,PseudoVoigtModel,SkewedVoigtModel,SplitLorentzianModel,DampedHarmonicOscillatorModel,ExponentialGaussianModel,ExpressionModel,SkewedGaussianModel
...
# 有两种 bins=71  /31(log)，bins=31(点平滑,但是太少了,作图时需要/2来与71结果一致)，拟合用bins=71（点多，略凌乱，获得的结果的概率接近TL）
        ...
        au, bu, cu = plt.hist((deltar-np.mean(deltar)) / (np.sqrt(2 * np.sum(deltar ** 2) / len(deltar))),71, histtype='bar', facecolor='yellowgreen', alpha=0.75, rwidth=1, density=True)  # au是counts，bu是deltar
        au /= len(deltar)
        ...
        AU, BU, CU = plt.hist((deltatheta-np.mean(deltatheta)) /(np.sqrt(2*np.sum(deltatheta**2)/len(deltatheta))), 71, histtype='bar', facecolor='blue',  alpha=0.75, rwidth=0.2, density=True)  #归一系数，但是range很奇怪
        AU /= len(deltar)
        AU_ind = np.where(AU == 0)
        AU = np.delete(AU,AU_ind)
        AU1 = np.array([math.log(x/2) for x in AU])
    ...
    left,bottom, width,height=[0.36,0.715,0.1,0.15]
    ...
    #
    #
    # ax3.scatter(expo1_x,expo1_y,s=10)
    # ax3.xaxis.set_major_locator(MultipleLocator(20))
    # ax3.yaxis.set_major_locator(MultipleLocator(1))
    # ax3.xaxis.set_minor_locator(MultipleLocator(10))
    # ax3.set_xlim(40,110)
    # ax3.set_ylim(1,3.5)
    # ax3.set_xlabel(r'$f$ (Hz)',fontsize=8)
    # ax3.set_ylabel(r'$\beta$',fontsize=8)
    # ax3.xaxis.set_label_coords(0.75, -0.25)
    # ax3.yaxis.set_label_coords(-0.15,0.5)
    # ax3.tick_params(axis="x",direction="in", labelsize=8)
    # ax3.tick_params(which='minor', direction='in')
    # ax3.tick_params(axis="y",direction="in", labelsize=8)
...
#             print(filename[i])
#
#             x_80_1 = np.arange(-0.3,0.2,0.001)*100.0
#             y_80_1 = [6.9 * np.exp(-(x_i + 13) ** 2 / 13 ** 2) for x_i in x_80_1 ]
#             ax2.plot(x_80_1/100.0, np.exp(y_80_1 + np.min(pdf_rot_dict[filename_1[i] + 'y'])),'--', color='orange', alpha=0.5)
#             x_80_2 = np.arange(-0.2,0.3,0.001)*100.0
#             y_80_2 = [6.9 * np.exp(-(x_i - 13) ** 2 / 13 ** 2) for x_i in x_80_2 ]
#             ax2.plot(x_80_2/100.0, np.exp(y_80_2 + np.min(pdf_rot_dict[filename_1[i] + 'y'])),'--', color='orange', alpha=0.5)
...
#     ax4.scatter(expo2_x,expo2_y,s=10)
...
#     ax4.set_xlabel(r'$f$ (Hz)',fontsize=8)
#     ax4.set_ylabel(r'$\Delta \Theta_0$ (rad)',fontsize=8)
#     ax4.xaxis.set_label_coords(0.75, -0.25)
#     ax4.yaxis.set_label_coords(-0.29,0.5)
#     ax4.tick_params(axis="x",direction="in", labelsize=8)
#     ax4.tick_params(which='minor', direction='in')
#     ax4.tick_params(axis="y",direction="in", labelsize=8)
...
#----energy distribution
    ...
    ax4.scatter(expo4_x,expo4_y,s=10)
    ...
    ax4.set_ylim(1,2)
    ax4.set_xlabel(r'$f$ (Hz)',fontsize=8)
    ax4.set_ylabel(r'$\beta$',fontsize=8)
...
#         # ax2.legend(loc='best')
    ...
    expo5_x=[60,80,100]
    ax5.scatter(expo5_x,expo5_y,s=10)
    ...
    ax5.set_xlim(50,110)
    ax5.set_ylim(1,7)
    ax5.set_xlabel(r'$f$ (Hz)',fontsize=8)
    ax5.set_ylabel(r'$\beta$',fontsize=8)
    ax5.xaxis.set_label_coords(0.5, -0.25)
    ax5.yaxis.set_label_coords(-0.15,0.5)
    ax5.tick_params(axis="x", direction="in", labelsize=8)
    ax5.tick_params(which='minor', direction='in')
    ax5.tick_params(axis="y", direction="in", labelsize=8)
